    ...
    try {
      const payload = isFolder
        ? {
            ...
            items: items.map((it) => ({
              file: it.label,
              output_urls: it.output_urls,
              output_image_url: it.imageUrl,
              measurements_csv: it.csvUrl,
              explanation: it.explanation,
              save_dir_url: it.save_dir_url,
            })),
          }
        : {
            mode: "file",
            item: {
              file: items[0].label,
              output_urls: items[0].output_urls,
              output_image_url: items[0].imageUrl,
              measurements_csv: items[0].csvUrl,
              explanation: items[0].explanation,
              save_dir_url: items[0].save_dir_url,
            },
          };

      const res = await fetch("http://127.0.0.1:8000/api/package-zip/", {
        method: "POST",
      ...
      if (data?.url) {
        ...
        alert("No download URL returned from server.");
      }
    } catch (e) {
    ...
    <section className="py-12 pt-2">
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-slate-200/50 max-w-5xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          {/* CSV for the active image + ZIP for all */}
          <div className="ml-auto flex items-center gap-2">
            ...
            >
              {downloading ? "Preparing ZIP…" : "Download ZIP (All)"}
            </button>
          </div>
        </div>
