...
#     else:
#         #raise Exception(f"LLM API error {response.status_code}: {response.text}")
#         return f"AI report is temporary unavailable for token limitation. Please check back later."
...
def get_generative_response(data):
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system", 
                "content": "You are a professional plant biologist writing clean, paragraph-based analytical reports. Never use bullet points or markdown headers. Output only the final response text without any internal thinking blocks."
            },
            {"role": "user", "content": prompt}
        ],
        "temperature": 0.5,
        "max_tokens": 500  # Prevents the model from rambling endlessly
    }
    ...
    if response.status_code == 200:
        ...
        # Strip out any <think>...</think> tracking blocks generated by Qwen reasoning
        cleaned_content = re.sub(r"<think>.*?</think>", "", raw_content, flags=re.DOTALL).strip()
        
        return cleaned_content if cleaned_content else raw_content
    else:
        return "AI report is temporarily unavailable for token limitation. Please check back later."